...
class AnalysisReagentRepositoryTest {
  ...
  @Autowired
  public AnalysisReagentRepositoryTest(
      AnalysisReagentRepository analysisReagentRepository,
  ...
  void setUp() {
    ...
    analysisReagent1 =
        AnalysisReagent.builder().analysis(analysis).reagent(reagent).quantity(1).build();
    analysisReagentRepository.save(analysisReagent1);
    // Create an analysisReagent
    analysisReagent2 =
        AnalysisReagent.builder().analysis(analysis).reagent(reagent).quantity(2).build();
  ...
  public void AnalysisReagentsRepository_findById_ReturnAnalysisReagent() {
    // Act
    Optional<AnalysisReagent> analysisReagentOptional =
        analysisReagentRepository.findById(analysisReagent1.getAnalysisReagentID());
    // Assert
    assertNotNull(analysisReagentOptional);
    assertTrue(analysisReagentOptional.isPresent());
    assertEquals(
        analysisReagent1.getAnalysisReagentID(),
        analysisReagentOptional.get().getAnalysisReagentID());
  }
  ...
  public void AnalysisReagentRepository_SaveAndFind_ReturnAnalysisReagent() {
    // Arrange
    AnalysisReagent analysisReagent =
        AnalysisReagent.builder().analysis(analysis).reagent(reagent).quantity(1).build();

    // Act
    AnalysisReagent savedAnalysisReagent = analysisReagentRepository.save(analysisReagent);
    Optional<AnalysisReagent> retrievedAnalysisReagent =
        analysisReagentRepository.findById(savedAnalysisReagent.getAnalysisReagentID());

    // Assert
    assertTrue(retrievedAnalysisReagent.isPresent());
    assertEquals(
        savedAnalysisReagent.getAnalysisReagentID(),
        retrievedAnalysisReagent.get().getAnalysisReagentID());
  }
  ...
  public void AnalysisReagentRepository_Delete_ReturnVoid() {
    // Arrange
    Optional<AnalysisReagent> savedAnalysisReagent =
        analysisReagentRepository.findById(analysisReagent1.getAnalysisReagentID());
    ...
    Optional<AnalysisReagent> deletedAnalysisReagent =
        analysisReagentRepository.findById(savedAnalysisReagent.get().getAnalysisReagentID());
  ...
  public void AnalysisReagentRepository_Update_ReturnUpdatedAnalysisReagent() {
    ...
    Optional<AnalysisReagent> updatedAnalysisReagent =
        analysisReagentRepository.findById(savedAnalysisReagent.get().getAnalysisReagentID());
